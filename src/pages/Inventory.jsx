...
function matchesSearch(p, q) {
  ...
  const fields = [
    ...
    p.material, p.gold_carat, p.occasion, p.diamond_purity,
...
export default function Inventory() {
  ...
  // Load products — filter by is_current (soft-delete pattern)
  ...
  const checkSKU = useCallback(async (sku, excludeId) => {
    const { data } = await db.from('products').select('id').eq('sku', sku).eq('is_current', true);
    if (!data) return true;
  ...
  const handleToggleStock = useCallback(async (product) => {
    ...
    const { error } = await db.from('products').update({ in_stock: newInStock }).eq('id', product.id);
    if (!error) {
      setProducts(prev => prev.map(p => p.id === product.id ? { ...p, in_stock: newInStock } : p));
      window._products = (window._products || []).map(p => p.id === product.id ? { ...p, in_stock: newInStock } : p);
