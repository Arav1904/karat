...
export default function Analytics() {
  ...
  const caratData = useMemo(() => {
    const m = {};
    prods.forEach(p => { if(p.gold_carat){ m[p.gold_carat]=(m[p.gold_carat]||0)+1; }});
    return Object.entries(m).map(([name,value])=>({name,value}));
  }, [prods.length]);

  // Stock
  const inStock = prods.filter(p=>p.in_stock===true).length;
  const soldOut = prods.filter(p=>p.in_stock===false).length;
