...
Supabase service role key inline as the value of `apikey` and
`Authorization: Bearer …` headers in every HTTP Request node that talks
to Supabase, and inside Code nodes as a string constant `SK`.
